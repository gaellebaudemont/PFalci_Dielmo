...
# download results
results_1AB_1tc = list.files(pattern = "*Priors.csv")
...
dfanalyses$chain <- as.character(c(rep(c(1,2,3,4), each = 4000)))
dfanalyses$iteration <- c(rep(seq(1,4000, 1), 4))
...
plot_chains <- ggarrange(plot_chains1[[1]], plot_chains1[[2]], plot_chains1[[3]], plot_chains1[[4]], 
 ...
 ggsave(filename = paste0(vec_malaria[Ab], "_1tc_chains_dielmo_Priors.jpeg"), plot_chains, width=12, height=8)
...
plot_VPC <- ggplot(SP_bins, aes(x = age, y = med, group = cohort)) +
...
ggsave(filename = paste0(vec_malaria[Ab], "_1tc_VPC_dielmo_Priors.jpeg"), plot_VPC, width=12, height=8)
